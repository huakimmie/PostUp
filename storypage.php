...
$cur_in = $_COOKIE['username'];
$qtitle = $_GET['title'];
$qtime = $_GET['time_stamp'];
$sql = "SELECT * FROM posts WHERE title = '$qtitle'";
$result = $db->query($sql);
$i = 0; 
while($res = $result->fetchArray(SQLITE3_ASSOC)){
	if($res['time_stamp'] == $qtime){
		$pusername = $res['username']; 
  		$ptitle = $res['title'];
  		$ppost = $res['post'];
		$ptime_stamp = $res['time_stamp'];
	}
  	$i++; 
}
...
<table class="organize">
	<tr>';
  $dtitle = str_replace("anapostrophe","'",$ptitle);
  echo'
	<th id="title">'.$dtitle.'</th>
	</tr>
	<tr>
	<td><strong>by: '.$pusername.'</strong></td>
	</tr>
	<tr >';
  $dpost = str_replace("anapostrophe","'",$ppost);
  echo'
	<td class="body"><br/>'.$dpost.'</td>
	</tr>
</table>
<table class="comments">
 <tr><th id="titlet">Post a Comment</th></tr>
      <tr><td>  <textarea cols= "70%" rows="10" id="carea" name="comments"></textarea></td></tr>
 	<tr><td>';
echo  '<input type="button" value="Submit" onclick="comment_form('."'$ptitle' , '$cur_in'".')"/>';
echo'   </td></tr> </table>
<div id="commentgen">
</div>';
echo '</div>';
